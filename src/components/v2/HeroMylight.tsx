...
const highlights = [
  "Installateur agréé Mylight",
  "Stockage virtuel illimité",
  "Suivi via app mobile",
  ...
  return (
    <section className="relative flex min-h-[100svh] items-center justify-center overflow-hidden">
      <div
        ...
        style={{ backgroundImage: `url(/images/hero-bg.jpg)`, transform: "translateZ(0)" }}
      />
      ...
      <div className="relative z-10 container mx-auto px-6 text-center py-[40px]">
        ...
        <a
          href="#offre"
          className="mb-4 inline-flex items-center gap-2 rounded-full border border-primary-foreground/20 bg-primary-foreground/10 px-4 py-2 text-xs sm:text-sm font-semibold text-primary-foreground backdrop-blur-sm transition-all hover:bg-primary-foreground/20 max-w-[90vw] text-center"
        >
          ...
          <span>
            Créneaux disponibles aux environs de <span className="font-bold">{city}</span>
          </span>
        </a>

        {/* Mylight badge */}
        <div className="mb-8 flex items-center justify-center">
          <div className="inline-flex items-center gap-3 rounded-full border border-primary-foreground/20 bg-primary-foreground/10 px-5 py-2.5 backdrop-blur-sm">
            <img src="/images/logo-mylight.png" alt="Mylight150" className="h-8 brightness-0 invert" />
        ...
        <h1 className="mx-auto mb-8 max-w-4xl text-4xl font-normal leading-tight text-primary-foreground md:text-5xl lg:text-6xl">
          ...
          <br />
          <span className="text-white drop-shadow-[0_2px_10px_rgba(0,0,0,0.4)]">
            stockage virtuel Mylight inclus.
          </span>
        </h1>

        {/* Subtitle */}
        <p className="mx-auto mb-4 max-w-2xl text-lg font-light text-primary-foreground/70">
          Panneaux photovoltaïques avec solution de stockage virtuel Mylight150 : produisez votre énergie, stockez le surplus sans batterie physique et consommez-le quand vous voulez.
        </p>
          ...
          {searchPhase === "idle" ? (
            <form onSubmit={handleSubmit} className="flex items-center">
              <input
                ...
                maxLength={5}
                className="flex-1 min-w-0 bg-transparent px-4 sm:px-6 py-5 text-foreground placeholder:text-muted-foreground outline-none"
              />
              <button
                type="submit"
                className="cta-pulse m-2 shrink-0 rounded-xl bg-primary px-5 sm:px-8 py-3.5 font-semibold text-primary-foreground transition-all hover:opacity-90"
              >
                Estimer mon projet
              </button>
            </form>
          ) : (
            <div className="animate-fade-in flex flex-col items-center gap-4 px-6 py-6">
